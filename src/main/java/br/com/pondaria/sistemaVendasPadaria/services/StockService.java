...
public class StockService {
    ...
    public StockItem addStockItemInStock(String barcode, BigDecimal quantity, MovementType movementType) {
        ...
        if(!searchedProduct.isPresent()) throw new IllegalArgumentException("Product no exists");
            ...
            if(stockItemSearched.isPresent()){
                return updateOldStock(searchedProduct.get(),quantity, stockItemSearched.get());
            }else{
                return addNewStock(searchedProduct.get(),quantity);
            }
        }
    }

    // Atualizado 07/08/2021
    private StockItem addNewStock(Product productAdd, BigDecimal quantity){
    ...
    //Atualizado 07/08/2021
    private StockItem updateOldStock(Product productAdd, BigDecimal quantity, StockItem oldStockItem){
